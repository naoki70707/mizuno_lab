...
def read_data(npm, nem, lines):
    ...
    # 要素番号の整列 <---------MESH2で要素の整列をすれば，以下は不要
    element = sorted(element.tolist(), key=lambda x:(x[1]))
    element = np.array(element)
    return node, element

    """
        1要素を構成する接点の並びを確認および修正する関数
    """
